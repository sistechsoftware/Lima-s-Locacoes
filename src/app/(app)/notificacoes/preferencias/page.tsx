...
export default async function PreferencesPage() {
  ...
  return <div className="space-y-4">
    <PageHeader title="Preferencias de notificacao" subtitle="Por tipo, funcao e dispositivo" />
    <Link href="/notificacoes" className="text-marca-600 underline">Voltar a central</Link>
    <PushDevices />
    <PushDiagnostico />
    <Section title="O que quero receber">
      ...
      <p className="mb-3 text-sm">Minhas funcoes: {roles.filter(r=>assignments.some(a=>a.user_id===user.id&&a.role===r.key)).map(r=>r.label).join(", ") || "nenhuma definida"}. Ser administrador nao inscreve voce em avisos.</p>
      <form action={savePreferences} className="space-y-3">
        {Object.entries(NOTIFICATION_TYPES).map(([type,label])=><label key={type} className="flex items-center justify-between gap-3 text-sm">{label}<select name={type} defaultValue={prefs.find(p=>p.type===type)?.mode??"auto"} className="campo max-w-56"><option value="auto">Automatico (funcao/atribuicao)</option><option value="on">Ativar explicitamente</option><option value="off">Desativar</option></select></label>)}
        <SubmitButton>Salvar preferencias</SubmitButton>
      </form>
    </Section>
    {user.role==="admin" && <>
      <Section title="Funcoes operacionais dos usuarios">
        <p className="text-sm text-stone-600 mb-3">Funcoes independentes das permissoes de acesso. Um usuario pode ter varias funcoes.</p>
        {users.map(u=><form key={u.id} action={saveRoles} className="border rounded-xl p-3 mb-3 space-y-2"><input type="hidden" name="user_id" value={u.id}/><h3 className="font-bold">{u.name} · {u.role}{!u.active&&" (inativo)"}</h3><div className="flex flex-wrap gap-3">{roles.map(r=><label key={r.key} className="text-sm flex items-center gap-1"><input type="checkbox" name="roles" value={r.key} defaultChecked={assignments.some(a=>a.user_id===u.id&&a.role===r.key)}/>{r.label}</label>)}</div><SubmitButton variant="secundario">Salvar funcoes</SubmitButton></form>)}
      </Section>
      <Section title="Regras gerais e antecedentes">
        <p className="text-sm text-stone-600 mb-3">Agendador no servidor: {lastRun?new Date(Number(lastRun.value)*1000).toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"}):"aguardando primeira execucao"}. Lembretes tem precisao de aproximadamente um minuto, sujeitos ao navegador e a rede.</p>
        <form action={savePushRules} className="space-y-4">
          <label className="flex gap-2"><input type="checkbox" name="push_enabled" defaultChecked={enabled?.value!=="0"}/>Habilitar envio push global (nao interrompe a central interna)</label>
