...
export default async function NotificationsPage({searchParams}:{searchParams:Promise<{tipo?:string;lidos?:string;antes?:string}>}) {
  ...
  return <div className="space-y-4">
    ...
    <form className="pilha-filtros"><select name="tipo" defaultValue={type} className="campo min-w-0 flex-1 basis-48 sm:flex-none sm:max-w-60"><option value="">Todos os tipos</option>{Object.entries(NOTIFICATION_TYPES).map(([key,label])=><option key={key} value={key}>{label}</option>)}</select><select name="lidos" defaultValue={sp.lidos??"todos"} className="campo min-w-0 flex-1 basis-36 sm:flex-none sm:max-w-44"><option value="todos">Lidas e nao lidas</option><option value="nao">Nao lidas</option></select><button className="w-full rounded-xl border px-4 py-2 sm:w-auto">Filtrar</button></form>
