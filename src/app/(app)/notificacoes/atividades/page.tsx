...
export default async function ActivitiesPage({searchParams}:{searchParams:Promise<{reserva?:string;tipo?:string;estado?:string}>}) {
  ...
  return <div className="space-y-4"><PageHeader title="Atividades operacionais" subtitle="Atribuicao direta, agenda integrada e separacao de materiais"/>
    <Link href="/notificacoes" className="text-marca-600 underline">Central de notificacoes</Link>
    ...
    {!!sp.reserva && <section className="cartao p-4"><h2 className="font-bold">Materiais fisicos da reserva</h2><p className="text-sm text-stone-500">Kits expandidos pela composicao salva na reserva. Concluir a separacao nao movimenta nem duplica estoque.</p><ul className="mt-2 space-y-1">{items.map(i=><li key={i.name}>{i.qty} × {i.name}</li>)}</ul><Link href={`/reservas/${Number(sp.reserva)}`} className="text-marca-600 underline text-sm">Conferir reserva e itens comerciais</Link></section>}
    {!activities.length&&<Empty>Nenhuma atividade neste filtro.</Empty>}
    {activities.map(a=><article key={a.id} id={`atividade-${a.id}`} className="cartao p-4 space-y-2"><h2 className="font-bold">{a.title}</h2><p className="text-sm">{a.scheduled_at.replace("T"," ")} · {a.status==="pending"?"Pendente":a.status==="completed"?"Concluida":"Cancelada"} · {a.assignee_name||"Equipe pelas funcoes"}</p>
      ...
      {manager&&<form action={updateActivity} className="flex flex-wrap gap-2"><input type="hidden" name="id" value={a.id}/><select aria-label="Responsavel exclusivo" name="assignee_id" className="campo max-w-xs" defaultValue={a.assignee_id??""}><option value="">Equipe pelas funcoes</option>{users.map(u=><option key={u.id} value={u.id}>{u.name}</option>)}</select><SubmitButton variant="secundario">Atribuir responsavel</SubmitButton></form>}
      {a.kind==="separacao"&&a.status==="pending"&&(manager||a.assignee_id===user.id||a.assignee_id===null&&roles.some(r=>r.role==="separador"))&&<form action={completeSeparation}><input type="hidden" name="id" value={a.id}/><SubmitButton variant="sucesso" confirm="Materiais e quantidades conferidos?">Concluir separacao</SubmitButton></form>}
    </article>)}
