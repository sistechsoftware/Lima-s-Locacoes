    ...
    <aside className="nao-imprimir hidden w-60 shrink-0 flex-col border-r border-areia-300 bg-white md:flex">
      <Link href="/dashboard" className="flex items-center gap-2.5 border-b border-areia-200 px-4 py-4">
        {logo ? (
          ...
          <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-terra-500 text-lg font-black text-white">
            L
          </span>
        )}
        <span className="min-w-0">
          <span className="block truncate text-sm font-bold leading-tight text-carvao-900">{company}</span>
          ...
          <Link
            ...
            className={`flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-medium transition ${
              active(pathname, n.href)
                ...
                : "text-carvao-700 hover:bg-areia-100"
            }`}
          >
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 sm:px-4">
        <Link href="/dashboard" className="flex items-center gap-2 md:hidden">
          {logo ? (
            ...
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-terra-500 text-base font-black text-white">
              L
            </span>
          )}
          <span className="max-w-[7.5rem] truncate text-sm font-bold text-carvao-900">{company}</span>
        </Link>
        ...
        <Link
          ...
          className="relative shrink-0 rounded-xl p-2 text-carvao-700 hover:bg-areia-100"
        >
          <Icon name="sino" />
          {unread > 0 && (
        ...
        <div className="relative shrink-0">
          <button
            onClick={() => setOpen((v) => !v)}
            className="flex h-9 w-9 items-center justify-center rounded-full bg-carvao-900 text-xs font-bold text-white"
          ...
          {open && (
            <>
              ...
              <div className="absolute right-0 z-20 mt-2 w-56 overflow-hidden rounded-xl border border-areia-300 bg-white shadow-lg">
                <div className="border-b border-areia-200 px-3 py-2.5">
                  <p className="text-sm font-bold text-carvao-900">{user.name}</p>
                  <p className="text-xs capitalize text-stone-500">{user.role}</p>
                </div>
                <Link
                  href="/configuracoes"
                  onClick={() => setOpen(false)}
                  className="flex items-center gap-2 px-3 py-2.5 text-sm hover:bg-areia-50"
                >
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="w-full rounded-xl border border-areia-300 bg-areia-50 py-2 pl-9 pr-3 text-sm outline-none focus:border-terra-400 focus:bg-white"
      />
    </form>
  );
}
    ...
    <>
      {sheet && (
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setSheet(false)}>
          ...
          >
            <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-areia-300" />
            <div className="grid grid-cols-3 gap-2">
              {EXTRA_NAV.map((n) => (
                <Link
                  key={n.href}
                  href={n.href}
                  className="flex flex-col items-center gap-1.5 rounded-xl border border-areia-200 px-2 py-3 text-center text-[0.7rem] font-semibold text-carvao-700"
                >
                  <Icon name={n.icon} className="h-5 w-5 text-terra-500" />
          ...
          {MOBILE_NAV.map((n) => (
            <Link
              ...
              className={`flex flex-col items-center gap-0.5 py-2 text-[0.63rem] font-semibold ${
                active(pathname, n.href) ? "text-terra-500" : "text-stone-500"
              }`}
            >
        ...
        {open &&
          ACOES.map((a) => (
            <Link
              key={a.href}
              href={a.href}
              className="flex items-center gap-2 rounded-full bg-white py-2.5 pl-3 pr-4 text-sm font-semibold text-carvao-900 shadow-lg"
            >
              <Icon name={a.icon} className="h-4 w-4 text-terra-500" />
              {a.label}
            </Link>
          ))}
        <button
          ...
          className="flex h-14 w-14 items-center justify-center rounded-full bg-terra-500 text-white shadow-xl transition active:scale-95"
        >
