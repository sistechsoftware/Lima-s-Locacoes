...
export default function AvailabilityFilter({ query, minutes, hidden = {}, fixed = false }: {
  ...
  return <section className="cartao p-4 space-y-2">
    <form className="grid grid-cols-1 gap-3 sm:grid-cols-2" key={query.queryString}>
      ...
      {!fixed && <>
        <label className="sm:col-span-1"><span className="rotulo">Data e horario da consulta</span><input required type="datetime-local" name="inicio" defaultValue={query.from} className="campo data-hora" /></label>
        <label className="sm:col-span-1"><span className="rotulo">Ate (opcional: consultar um intervalo)</span><input type="datetime-local" name="fim" defaultValue={query.to === query.from ? "" : query.to} min={query.from} className="campo data-hora" /></label>
      </>}
      <label className="flex items-center gap-2 text-sm sm:col-span-2"><input type="checkbox" name="preparo" value="1" defaultChecked={query.considerPreparation} />Considerar tempo de deslocamento e higienizacao</label>
